...
namespace FamilyNotes.Services
{
    ...
    "iknowu", "SqJoJfVI0DlaeL0rrsnprGvovrSkzPT03RwVAY7l01YeDImxA7gH4AnvbFTHUFs1qbDmIt389pfrSWgjsh5toQ==");
        ...
        public static async Task DownloadFaceImageAsFile(Face face, StorageFile DestinationStorageFile)
        {
            CloudBlobContainer container = await GetContainer();
            CloudBlob blob = container.GetBlobReference(face.Id + ".jpg");
            //copy blob from cloud to local gallery
            await blob.DownloadToFileAsync(DestinationStorageFile);
        }
